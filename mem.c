...
void * best_fit_allocator(unsigned int size) {
	...
	struct mem_region *before_region=NULL;
	while(current_region != NULL)
	{
		if (size <= current_region->size) {
			found = 1;
			if(before_region==NULL)
			{
				before_region = current_region;
			}
			else if(before_region->size>current_region->size )
			{
				before_region=current_region;
			}
	...
	if(found==1)
	{
		struct mem_region *now_region=before_region;
		struct mem_region*tmp=(struct mem_region*)malloc(sizeof(struct mem_region));
		tmp->pointer = now_region->pointer;
		tmp->size=size;
		...
		if(now_region->size==size)
		{
			if (now_region == free_regions)
			{
			...
			{
				if (now_region->prev != NULL) {
					now_region->prev->next = now_region->next;
				}
				if (now_region->next != NULL) {
					now_region->next->prev = now_region->prev;
				}
			}
		...
		{
			now_region->pointer += size;
			now_region->size -= size;
		}
		return tmp->pointer;
