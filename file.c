...
int translate(address_t virtual, address_t * physical) {
	...
	// chuyen doi dia chi	page	offset
	index_t index_temp =  get_index(virtual);
	index_t offset_temp=  get_offset(virtual);


	// duyet tim kiem		thay hoc ko
	int notfound = 1;		// bien de bieu thi viec tim thay vung nho
	int i=0;
	for ( i = 0; i < TLB_SIZE; i++)
	{
		if(index_temp == tlb[i].virtual)
		{
			notfound=0;
			(*physical)=tlb[i].physical;
			(*physical)=(*physical) << OFF_LEN;
			(*physical)=(*physical) | offset_temp;
			tlb[i].count++;
			return 1;
	...
	{
		if(index_temp==page_table[i].virtual)
		{
			notfound=0;
			(*physical)=page_table[i].physical;
			(*physical)=(*physical) << OFF_LEN;
			(*physical)=(*physical) | offset_temp;
			page_table[i].count++;

			struct tlb_slot_t *page_change= get_least_accessed_slot();
